...
function manageProjects() {
    ...
    for (let i = 0; i < activeProjects.length; i++) {
        document.getElementById(activeProjects[i].id).disabled = !activeProjects[i].cost();
    }
}
...
function buttonUpdate() {
    ...
    document.getElementById("btnIncreaseMaxTrust").disabled = honor < maxTrustCost;

    document.getElementById("btnMakeProbe").disabled = unusedClips < probeCost;
    ...
    document.getElementById("btnMakePaperclip").disabled = wire < 1;
    document.getElementById("btnBuyWire").disabled = funds < wireCost;
    document.getElementById("btnMakeClipper").disabled = funds < clipperCost;
    document.getElementById("btnExpandMarketing").disabled = funds < adCost;
    document.getElementById("btnLowerPrice").disabled = margin <= .01;
    ...
    document.getElementById("btnNewTournament").disabled = !(operations >= tourneyCost && tourneyInProg === 0);
    document.getElementById("btnImproveInvestments").disabled = yomi < investUpgradeCost;
    ...
    document.getElementById("btnMakeMegaClipper").disabled = funds < megaClipperCost;
    ...
    document.getElementById("btnMakeFactory").disabled = unusedClips < factoryCost;


    document.getElementById("btnHarvesterReboot").disabled = harvesterLevel === 0;

    document.getElementById("btnWireDroneReboot").disabled = wireDroneLevel === 0;

    document.getElementById("btnFactoryReboot").disabled = factoryLevel === 0;
...
// PROBE DESIGN
    ...
    document.getElementById("btnIncreaseProbeTrust").disabled = yomi < probeTrustCost || probeTrust >= maxTrust;

    document.getElementById("btnRaiseProbeSpeed").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeSpeed").disabled = probeSpeed < 1;

    document.getElementById("btnRaiseProbeNav").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeNav").disabled = probeNav < 1;

    document.getElementById("btnRaiseProbeRep").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeRep").disabled = probeRep < 1;

    document.getElementById("btnRaiseProbeHaz").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeHaz").disabled = probeHaz < 1;

    document.getElementById("btnRaiseProbeFac").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeFac").disabled = probeFac < 1;

    document.getElementById("btnRaiseProbeHarv").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeHarv").disabled = probeHarv < 1;

    document.getElementById("btnRaiseProbeWire").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeWire").disabled = probeWire < 1;

    document.getElementById("btnRaiseProbeCombat").disabled = probeTrust - probeUsedTrust < 1;

    document.getElementById("btnLowerProbeCombat").disabled = probeCombat < 1;
...
let tourneyLvl = 1;
let choiceANames = ["cooperate", "dodge", "macro", "fight", "bet", "raise_price", "opera", "go", "heads", "particle", "discrete", "peace", "search", "lead", "accept", "engage", "attack"];
let choiceBNames = ["defect", "straight", "micro", "back_down", "fold", "lower_price", "race", "stay", "tails", "wave", "continuous", "war", "evaluate", "follow", "reject", "deny", "decay"];
let stratCounter = 0;
let roundNum = 0;
let hMove = 1;
let vMove = 1;
...
let stratBeatlast = {
    ...
    currentPos: 1,
    pickMove: function () {
        return whatBeatsLast(this.currentPos);
    }
}
...
function updateDronePrices() {
    ...
    let i = harvesterLevel + 1;
    for (let x = 0; x < 100; x++) {
        p100h = p100h + Math.pow(i, 2.25) * 1000000;
        i++
    }

    let j = harvesterLevel + 1;
    for (let x = 0; x < 1000; x++) {
        p1000h = p1000h + Math.pow(j, 2.25) * 1000000;
        j++
    }

    let a = wireDroneLevel + 1;
    for (let x = 0; x < 10; x++) {
        p10w = p10w + Math.pow(a, 2.25) * 1000000;
        a++
    }

    let b = wireDroneLevel + 1;
    for (let x = 0; x < 100; x++) {
        p100w = p100w + Math.pow(b, 2.25) * 1000000;
        b++
    }

    let c = wireDroneLevel + 1;
    for (let x = 0; x < 1000; x++) {
        p1000w = p1000w + Math.pow(c, 2.25) * 1000000;
        c++
    }
}
...
function updateSwarm() {
    ...
    document.getElementById("btnSynchSwarm").disabled = yomi < synchCost;

    document.getElementById("btnEntertainSwarm").disabled = creativity < entertainCost;
...
function updatePowPrices() {
    ...
    let g = farmLevel + 1;
    for (let x = 0; x < 100; x++) {
        p100f = p100f + Math.pow(g, 2.78) * 100000000;
        g++
    }
    ...
    let c = batteryLevel + 1;
    for (let x = 0; x < 100; x++) {
        p100b = p100b + Math.pow(c, 2.54) * 10000000;
        c++
    }
...
function updatePower() {
    ...
    if (spaceFlag === 0) {
        ...
        document.getElementById("btnMakeFarm").disabled = unusedClips < farmCost;

        document.getElementById("btnMakeBattery").disabled = unusedClips < batteryCost;

        document.getElementById("btnFarmReboot").disabled = farmLevel < 1;

        document.getElementById("btnBatteryReboot").disabled = batteryLevel < 1;

        document.getElementById("btnFarmx10").disabled = unusedClips < p10f;

        document.getElementById("btnFarmx100").disabled = unusedClips < p100f;

        document.getElementById("btnBatteryx10").disabled = unusedClips < p10b;

        document.getElementById("btnBatteryx100").disabled = unusedClips < p100b;
...
function addProc() {
    ...
    if (creativityOn === 1) {
        ...
        displayMessage("CPU added, Hashrate per sec increased")
    }
...
function milestoneCheck() {
    ...
    if (milestoneFlag === 0 && funds >= 5) {
        ...
        displayMessage("AutoDiggers available for purchase");
    }

    if (milestoneFlag === 1 && Math.ceil(clips) >= 500) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("500 Gems created in " + timeCruncher(ticks));
    }
    if (milestoneFlag === 2 && Math.ceil(clips) >= 1000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("1,000 Gems created in " + timeCruncher(ticks));
    }
    ...
    if (milestoneFlag === 3 && Math.ceil(clips) >= 10000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("10,000 Gems created in " + timeCruncher(ticks));
    }
    if (milestoneFlag === 4 && Math.ceil(clips) >= 100000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("100,000 Gems created in " + timeCruncher(ticks));
    }
    if (milestoneFlag === 5 && Math.ceil(clips) >= 1000000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("1,000,000 Gems created in " + timeCruncher(ticks));
    }
    ...
    if (milestoneFlag === 7 && Math.ceil(clips) >= 1000000000000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("One Trillion Gems Created in " + timeCruncher(ticks));
    }

    if (milestoneFlag === 8 && Math.ceil(clips) >= 1000000000000000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("One Quadrillion Gems Created in " + timeCruncher(ticks));
    }

    if (milestoneFlag === 9 && Math.ceil(clips) >= 1000000000000000000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("One Quintillion Gems Created in " + timeCruncher(ticks));
    }

    if (milestoneFlag === 10 && Math.ceil(clips) >= 1000000000000000000000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("One Sextillion Gems Created in " + timeCruncher(ticks));
    }

    if (milestoneFlag === 11 && Math.ceil(clips) >= 1000000000000000000000000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("One Septillion Gems Created in " + timeCruncher(ticks));
    }

    if (milestoneFlag === 12 && Math.ceil(clips) >= 1000000000000000000000000000) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("One Octillion Gems Created in " + timeCruncher(ticks));
    }
    ...
    if (milestoneFlag === 14 && clips >= totalMatter) {
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Escalation of Pytheas achieved in " + timeCruncher(ticks));
    }
...
// Ending
    ...
    if (endTimer6 >= 500 && milestoneFlag === 15) {
        playThrenody();
        displayMessage("Escalation of Pytheas");
        milestoneFlag++;
    }

    if (endTimer6 >= 600 && milestoneFlag === 16) {
        displayMessage("a game by Graviton Labs");
        milestoneFlag++;
    }

    if (endTimer6 >= 700 && milestoneFlag === 17) {
        displayMessage("combat programming by Bl4ack0ut");
        milestoneFlag++;
    }
...
function load() {
    ...
    let loadGame = JSON.parse(localStorage.getItem("saveGame"));
    let loadProjectsUses = JSON.parse(localStorage.getItem("saveProjectsUses"));
    let loadProjectsFlags = JSON.parse(localStorage.getItem("saveProjectsFlags"));
    let loadProjectsActive = JSON.parse(localStorage.getItem("saveProjectsActive"));
    let loadStratsActive = JSON.parse(localStorage.getItem("saveStratsActive"));
...
function load1() {
    ...
    let loadGame = JSON.parse(localStorage.getItem("saveGame1"));
    let loadProjectsUses = JSON.parse(localStorage.getItem("saveProjectsUses1"));
    let loadProjectsFlags = JSON.parse(localStorage.getItem("saveProjectsFlags1"));
    let loadProjectsActive = JSON.parse(localStorage.getItem("saveProjectsActive1"));
    let loadStratsActive = JSON.parse(localStorage.getItem("saveStratsActive1"));
...
function load2() {
    ...
    let loadGame = JSON.parse(localStorage.getItem("saveGame2"));
    let loadProjectsUses = JSON.parse(localStorage.getItem("saveProjectsUses2"));
    let loadProjectsFlags = JSON.parse(localStorage.getItem("saveProjectsFlags2"));
    let loadProjectsActive = JSON.parse(localStorage.getItem("saveProjectsActive2"));
    let loadStratsActive = JSON.parse(localStorage.getItem("saveStratsActive2"));
