...
function quantumCompute(){
    qClock = qClock+.01;
    for (var i = 0; i<qChipsElements.length; i++){
...
function buttonUpdate() {
    ...
    btnMakePaperclipElement.disabled = wire < 1;
    btnBuyWireElement.disabled = funds < wireCost;
    btnMakeClipperElement.disabled = funds < clipperCost;
    btnExpandMarketingElement.disabled = funds < adCost;
    btnLowerPriceElement.disabled = margin <= .01;
    ...
    btnNewTournamentElement.disabled = !(operations >= tourneyCost && tourneyInProg === 0);
    btnImproveInvestmentsElement.disabled = yomi < investUpgradeCost;
    ...
    btnMakeMegaClipperElement.disabled = funds < megaClipperCost;
    ...
    btnMakeFactoryElement.disabled = unusedClips < factoryCost;


    btnHarvesterRebootElement.disabled = harvesterLevel === 0;

    btnWireDroneRebootElement.disabled = wireDroneLevel === 0;

    btnFactoryRebootElement.disabled = factoryLevel === 0;
...
// PROBE DESIGN
    ...
    btnIncreaseProbeTrustElement.disabled = yomi < probeTrustCost || probeTrust >= maxTrust;
    ...
    btnLowerProbeHaz.disabled = probeHaz < 1;
...
function newTourney() {
    ...
    tourneyDisplayElement.innerHTML = "Pick strategy, run simulation, gain intel";
...
function declareWinner() {
    ...
    if (pick < 10) {
        ...
        if (milestoneFlag < 15) {
            ...
            displayMessage(strats[pick].name + " scored " + strats[pick].currentScore + " and beat " + bB + " " + w + " in the simulation. Intel increased by " + strats[pick].currentScore * yomiBoost * beatBoost);
...
function updateDroneButtons() {
    ...
    btnMakeHarvesterElement.disabled = unusedClips < harvesterCost;

    btnHarvesterx10Element.disabled = unusedClips < p10h;

    btnHarvesterx100Element.disabled = unusedClips < p100h;

    btnHarvesterx1000Element.disabled = unusedClips < p1000h;

    btnMakeWireDroneElement.disabled = unusedClips < wireDroneCost;

    btnWireDronex10Element.disabled = unusedClips < p10w;

    btnWireDronex100Element.disabled = unusedClips < p100w;

    btnWireDronex1000Element.disabled = unusedClips < p1000w;
...
function updateSwarm() {
    ...
    btnSynchSwarmElement.disabled = yomi < synchCost;

    btnEntertainSwarmElement.disabled = creativity < entertainCost;
...
function updatePower() {
    ...
    if (humanFlag === 0 && spaceFlag === 0) {
        ...
        btnMakeFarmElement.disabled = unusedClips < farmCost;

        btnMakeBatteryElement.disabled = unusedClips < batteryCost;

        btnFarmRebootElement.disabled = farmLevel < 1;

        btnBatteryRebootElement.disabled = batteryLevel < 1;

        btnFarmx10Element.disabled = unusedClips < p10f;

        btnFarmx100Element.disabled = unusedClips < p100f;

        btnBatteryx10Element.disabled = unusedClips < p10b;

        btnBatteryx100Element.disabled = unusedClips < p100b;
...
function updateStats() {
    ...
    if (wire === 1) {
        inchSpanElement.innerHTML = "KG";
    } else {
        inchSpan.innerHTML = "KGs";
    }
...
// Ending
    ...
    if (endTimer6 >= 800 && milestoneFlag === 18) {
        displayMessage("Android says 'yes'. ");
        milestoneFlag++;
    }
